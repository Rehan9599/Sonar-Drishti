...
export function connectToJob(jobId, handlers = {}) {
  if (MOCK) {
    ...
    const interval = setInterval(() => {
      if (i < MOCK_STREAM_DETECTIONS.length) {
        handlers.onPartial?.({ tile_index: i, detections: [MOCK_STREAM_DETECTIONS[i]] });
        i++;
      } else {
  ...
  const socket = new WebSocket(`${base}/ws/jobs/${jobId}/`);
